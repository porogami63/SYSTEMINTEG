...
try {
    ...
    if ($cert) {
        ...
        try {
            $db->execute("INSERT INTO verifications (cert_id, ip_address, user_agent) VALUES (?, ?, ?)", [$cert['id'], $_SERVER['REMOTE_ADDR'] ?? 'unknown', $_SERVER['HTTP_USER_AGENT'] ?? '']);
        } catch (Exception $e) {
            // ignore
        }
        ?>
    ...
    // On DB error, show a simple message
