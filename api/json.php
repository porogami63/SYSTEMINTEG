...
if (empty($cert_id)) {
    echo JsonHelper::encode(['status' => 'error', 'message' => 'cert_id parameter is required']);
    exit;
}
...
try {
    ...
    if ($cert) {
        ...
        echo JsonHelper::encode($response);
    } else {
        echo JsonHelper::encode(['status' => 'error', 'message' => 'Certificate not found']);
    }
} catch (Exception $e) {
    echo JsonHelper::encode(['status' => 'error', 'message' => 'Server error: ' . $e->getMessage()]);
}
?>
